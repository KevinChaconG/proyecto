...
// Compañeros, esta ruta ACTUALIZA el estado de una matrícula (solo admin)
router.put('/matriculas/:id_matricula/estado', async (req, resp) => {
  try {
    const { id_matricula } = req.params;
    const { estado } = req.body;

    const estadosValidos = ['activa', 'completada', 'retirada'];
    if (!estado || !estadosValidos.includes(String(estado).toLowerCase())) {
      return resp.status(400).json({ mensaje: 'Estado inválido' });
    }
    ...
    matricula.estado = String(estado).toLowerCase();
    await matricula.save();

    return resp.json({ mensaje: 'Estado actualizado exitosamente', matricula });
  } catch (error) {
    console.log(error);
    resp.status(500).json({ mensaje: 'Error al actualizar estado' });
  }
});
