...
const [showModalEditarMatricula, setShowModalEditarMatricula] = useState(false);
const [matriculaEdit, setMatriculaEdit] = useState<any | null>(null);
const [estadoEdit, setEstadoEdit] = useState<"activa" | "completada" | "retirada">("activa");

const abrirModalEditarMatricula = (mat: any) => {
  setMatriculaEdit(mat);
  setEstadoEdit((mat.estado || "activa").toLowerCase());
  setShowModalEditarMatricula(true);
};

const actualizarEstadoMatricula = async () => {
  if (!matriculaEdit) return;

  try {
    const resp = await fetch(`http://localhost:5050/matricula/matriculas/${matriculaEdit.id_matricula}/estado`, {
  method: "PUT",
  ...
  body: JSON.stringify({ estado: estadoEdit }),
    ...
    if (resp.ok) {
      ...
      setShowModalEditarMatricula(false);
...
const eliminarMatricula = async (id: number) => {
  ...
  try {
    const resp = await fetch(
      `http://localhost:5050/matricula/matriculas/${id}`,
      {
        method: "DELETE",
      }
    );
  ...
  // ====== REPORTES ======
const matriculadosPorCurso = React.useMemo(() => {
  const conteo: Record<string, number> = {};

  for (const m of matriculas) {
    const curso = m.nombre_asignatura || "N/A";
...
const matriculasPorEstado = React.useMemo(() => {
  const conteo: Record<string, number> = { activa: 0, completada: 0, retirada: 0 };
  ...
  return {
    labels: ["Activa", "Completada", "Retirada"],
    datasets: [
      {
        label: "Matrículas por estado",
        data: [conteo.activa, conteo.completada, conteo.retirada],
      },
    ],
  };
}, [matriculas]);
...
<td>
  <button
    className="btn btn-sm btn-outline-primary me-2"
    onClick={() => abrirModalEditarMatricula(mat)}
  >
    ✏️ Editar
  </button>
      ...
      <div className="row g-4">
        <div className="col-12 col-lg-6">
          <div className="card border-0 shadow-sm h-100">
            <div className="card-body">
              <h5 className="card-title">Estudiantes matriculados por curso</h5>
              {matriculas.length === 0 ? (
                <p className="text-muted mb-0">No hay datos de matrículas.</p>
              ) : (
                <Bar data={matriculadosPorCurso} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="col-12 col-lg-6">
          <div className="card border-0 shadow-sm h-100">
            <div className="card-body">
              <h5 className="card-title">Matrículas por estado</h5>
              {matriculas.length === 0 ? (
                <p className="text-muted mb-0">No hay datos de matrículas.</p>
              ) : (
                <Bar data={matriculasPorEstado} />
              )}
            </div>
          </div>
        </div>
      ...
      <div className="d-flex">
        ...
        <aside className="sidebar">
          ...
          <ul className="menu-list">
            ...
            <li className="menu-item">
  <button
    className={`menu-link ${seccionActiva === "reportes" ? "active" : ""}`}
...
{showModalEditarMatricula && (
  <div className="modal-overlay" onClick={() => setShowModalEditarMatricula(false)}>
    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
      <div className="p-4">
        <h3 style={{ color: "#2F4858" }}>✏️ Editar Estado de Matrícula</h3>
        <hr />

        <div className="mb-3">
          <label className="form-label fw-bold">Estudiante</label>
          <input className="form-control" value={matriculaEdit?.estudiante || ""} disabled />
        </div>

        <div className="mb-3">
          <label className="form-label fw-bold">Curso</label>
          <input className="form-control" value={matriculaEdit?.nombre_asignatura || ""} disabled />
        </div>
        ...
        <div className="d-flex gap-2 mt-4">
          ...
          <button
            ...
            onClick={actualizarEstadoMatricula}
          >
            💾 Guardar
          </button>
        </div>
